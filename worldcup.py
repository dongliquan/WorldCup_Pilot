...
native WebKit window pointing at the single-page UI. Local / personal use only.

Dev run:
    pip3 install -r requirements.txt
    python3 worldcup.py

Build a .app bundle (see build.sh):
...
def set_dock_icon():
    """Dev mode shows Python's default Dock icon; the bundled icon.icns only
    applies to the built .app. Set the running app's Dock icon at runtime."""
    try:
        from AppKit import NSApplication, NSImage
        for name in ("icon.png", "icon_1024.png"):
            p = os.path.join(app_root(), "assets", name)
            if os.path.exists(p):
...
class Api:
    ...
    def notify(self, title, body):
        # native macOS notification (no extra deps)
        try:
...
def main():
    ...
    # set the Dock icon once the Cocoa event loop is up (otherwise pywebview
    # overrides it during launch); the bundled .app uses icon.icns instead.
